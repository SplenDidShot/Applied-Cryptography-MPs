...
- Create a proof that C = g^a*h^r, and a is in the range [0,64).
    ...
    Zk{ (a): C = g^a*h^r and  0 <= a <= 63 }

  Hint: You can implement this by creating commitments to the binary expansion
    of A, and then proving the following:

    Zk{ (b0, b1, ... b4,b5, r0, r1, ..., r4, r5): A = g^(b0 + 2*b1 + ... + 16*b4 + 32*b5)*g^(r0 + 2*r1 + ... + 16*r4 + 32*r5)*h(r')
                                    and  (C0 = g^(b0) h^r0), C1 = g^(2*b1)h^r1, C2 = g^(4*b2)h^r2....
...
def range_prover(a, r, C, rnd_bytes=os.urandom, RO=sha2):
    ...
    C_temp = C
    for bit in bin_a:
        (Comm, rand) = make_pedersen_commitment(bit, rnd_bytes)
        C_arr.append(Comm)
        r_arr.append(rand)
    ...
    for i in range(len(bin_a)):
        A = C_arr[i]
        B = A - G
        prf = OR_prover(A, B, r_arr[i], rnd_bytes, RO)
        prf_arr.append(prf)
        C_temp = C_temp - (2 ** i) * A

    dlog_res = dlog_prover(C_temp, r)

    return (C_arr, prf_arr, dlog_res)
...
def range_verifier(C, prf, rnd_bytes=os.urandom, RO=sha2):
    ...
    for i in range(len(C_temp)):
        ...
        B = A - G
        C = C - (2 ** i) * A
        assert OR_verifier(A, B, prf_arr[i])

    assert dlog_verifier(C, dlog_res)
    return True
