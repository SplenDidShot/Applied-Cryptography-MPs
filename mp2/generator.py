    ...
    def garble(self):
        ...
        for gid in self.gates:
            ...
            for i in range(len(table)):
                temp.append(specialEncryption(bytes.fromhex(labels[inp[0]][int(i/2)]), 
                    specialEncryption(bytes.fromhex(labels[inp[1]][int(i%2)]), bytes.fromhex(labels[out[0]][table[i]]))).hex())
            shuffle(temp)
