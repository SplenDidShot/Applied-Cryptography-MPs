...
class GarbledCircuitEvaluator(BooleanCircuit):
    ...
    def garbled_evaluate(self, inp):
        ...
        for gid in self.sorted_gates:
            ...
            inp = gate["inp"]
            out = gate["out"][0]
            k = bytes.fromhex(self.wire_labels[inp[0]])
            for r in table:
                c = bytes.fromhex(r)
                first = specialDecryption(k,c)
                if first!=None:
                    j = bytes.fromhex(self.wire_labels[inp[1]])
                    second = specialDecryption(j,first)
                    if second!=None:
                        #print(out,second.hex())
                        self.wire_labels[out] = second.hex()
