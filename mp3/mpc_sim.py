...
def get_random_bit(context):
    ...
    # TODO: Your code goes here
    while 1:
        rr = beaver_multiply(context,r,r)
        RR = context.open_share(rr)
        if RR.n != 0: break
    r1 = prime_mod_sqrt(RR.n,Fp.p)
    b = (r/Fp(r1) + 1)/Fp(2)
...
def compute_powers(context, x, k):
    ...
    # TODO: your code goes here
    invrs = [compute_inverse(context,i) for i in rs]
    temps = []
    temps.append(Fp(0))
    #open share all intermediate values
    for i in range(1,k+1):
        temp = beaver_multiply(context,rs[i-1],x)
        d = beaver_multiply(context,temp,invrs[i])
        temps.append(context.open_share(d))
        
    d01 = Fp(1)
    #determine the power of x step by step by interating through temps
    for j in range(1,k+1):
        d01*=temps[j]
        x_pow = d01*beaver_multiply(context,invrs[0],rs[j])
        xis.append(x_pow)
    return xis
...
def random_extract(context, avals):
    ...
    #make points with Fp,aval tuples
    points = [(Fp(i+1), avals[i]) for i in range(len(avals))]
    phi = Poly.interpolate(points)
    truth = [phi(Fp(context.n+i+1)) for i in range(context.n-context.f)]
    return truth
