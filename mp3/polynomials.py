...
if __name__ == '__main__':
    Fp = FiniteField(p=59,m=1) # Finite field is specified with a prime p
...
def eval_poly(f, x):
    ...
    deg = 0
    p = []
    #populate list p for easier operation
...
def interpolate(Poly, points):
    ...
    p = 0
    for (x,y) in points:
        p += y*(lagrange_polynomial(Poly, xs, x))
        print(p)
    return Poly(p)
...
if __name__ == '__main__':
    ...
    print("-----------")
    pts = [(0,0), (1,28), (2,54), (3,29), (4,39), (5,10)]
    p_test = interpolate(Poly, pts)
    print(p_test)
